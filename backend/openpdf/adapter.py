"""OpenPDF render, quarantine, and independent publication gate."""
...
_WORKER_ROOT = _REPO_ROOT / "workers" / "openpdf"
_DEFAULT_JAR = _WORKER_ROOT / "target" / "claros-openpdf-worker-0.1.0-SNAPSHOT-all.jar"
...
class OpenPdfConfigurationError(RuntimeError):
    """The selected OpenPDF runtime cannot start safely."""
...
class OpenPdfRuntime:
    ...
    def __post_init__(self) -> None:
        if (
            ...
            raise ValueError("OpenPDF limits must be positive")
        if self.jvm_heap_mib > 1_024:
            raise ValueError("OpenPDF heap must remain bounded")
...
class OpenPdfWorkerExportEngine:
    ...
    def __init__(
        ...
        *,
        ...
        runtime: OpenPdfRuntime | None = None,
    ) -> None:
        self._control = control_engine or DocumentProcessExecutor()
        self._runtime = runtime or OpenPdfRuntime()
    ...
    def verify_runtime(self) -> None:
        """Fail startup when the selected renderer or validator runtime is missing."""

        jar = self._runtime.jar_path
        if not jar.is_file() or jar.is_symlink():
            raise OpenPdfConfigurationError(f"OpenPDF worker artifact is unavailable: {jar}")
        try:
            self._font_sha256()
        except DocumentEngineError as error:
            raise OpenPdfConfigurationError(error.safe_message) from error
        java = _resolve_executable(self._runtime.java_command, "Java 21")
        qpdf = self._qpdf_path()
        java_result = subprocess.run(  # noqa: S603
            [str(java), "-version"],
        ...
        java_version = java_result.stdout.decode("utf-8", errors="replace")
        match = re.search(r'version "(?P<major>\d+)', java_version)
        if java_result.returncode != 0 or match is None or int(match.group("major")) < 21:
            raise OpenPdfConfigurationError("OpenPDF export requires Java 21 or newer")
        qpdf_result = subprocess.run(  # noqa: S603
            [str(qpdf), "--version"],
            stdin=subprocess.DEVNULL,
            stdout=subprocess.PIPE,
            stderr=subprocess.STDOUT,
        ...
        if qpdf_result.returncode != 0:
            raise OpenPdfConfigurationError("OpenPDF export requires a working qpdf executable")
    ...
    async def _validate(
        ...
        if pdfbox_status.job_id != job.job_id or pdfbox_status.page_count != worker.output_pages:
            raise document_error("invalid_export")
        if not pdfbox_status.placement_exact or not pdfbox_status.source_preserved:
            raise document_error("invalid_export")
        if pdfbox_status.rendered_pages != worker.output_pages:
            raise document_error("invalid_export")
    ...
    async def _run(
        ...
        if remaining <= 0:
            raise DocumentExecutionTimeout("OpenPDF export exceeded its deadline")
        ...
        try:
            ...
            while process.poll() is None:
                ...
                if time.monotonic() >= deadline:
                    ...
                    raise DocumentExecutionTimeout("OpenPDF process exceeded its deadline")
                time.sleep(0.01)
            peak_rss = _max_optional(peak_rss, _process_rss(process.pid))
    ...
    def _pdfbox_command(self, job_root: Path) -> tuple[str, ...]:
        ...
        return (
            ...
            "-cp",
            str(self._runtime.jar_path),
            "org.claros.openpdf.PdfBoxValidatorMain",
            "--job-dir",
            str(job_root),
        )
    ...
    def _qpdf_path(self) -> Path:
        ...
        if configured is not None:
            ...
            raise OpenPdfConfigurationError(
                f"Configured qpdf executable is unavailable: {configured}"
            )
        found = shutil.which("qpdf")
        if found:
            return Path(found).resolve()
        raise OpenPdfConfigurationError(
            "OpenPDF export requires qpdf on PATH or CLAROS_OPENPDF_QPDF_PATH"
        )
...
def select_pdf_engine(
    ...
    *,
    ...
    """Select one renderer explicitly without failure fallback."""

    if engine == "current":
        return current
    if engine != "openpdf":
        raise ValueError("CLAROS_PDF_ENGINE must be current or openpdf")
    selected = OpenPdfWorkerExportEngine(control_engine=current, runtime=runtime)
    if verify_runtime:
        selected.verify_runtime()
    return selected
...
def _manifest(
    ...
    *,
    ...
    return DocumentExportManifest(
        exporter_version="claros-openpdf-v1",
...
def _resolve_executable(command: str, label: str) -> Path:
    ...
    if found:
        return Path(found).resolve()
    raise OpenPdfConfigurationError(f"OpenPDF export requires {label}")
